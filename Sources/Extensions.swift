...
public extension ThemeWrapper where Base: UITabBarItem {
    var image: ThemeProvider<UIImage>? {
        set(new) {
            let baseItem = self.base
            setupBarItemThemeProperty(barItem: self.base, key: "UITabBarItem.image", provider: new) {[weak baseItem] (style) in
                baseItem?.image = new?.provider(style)
            }
        }
        get { return self.base.providers["UITabBarItem.image"] as? ThemeProvider<UIImage> }
    }
